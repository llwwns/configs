...
cmp.setup({snippet = {expand = _1_}, mapping = mapping, window = {completion = cmp.config.window.bordered({border = "single"}), documentation = cmp.config.window.bordered({border = "single"})}, sources = {{name = "nvim_lsp_signature_help"}, {name = "luasnip"}, {name = "nvim_lsp"}, {name = "nvim_lua"}, {name = "path"}, {name = "buffer"}, {name = "words", max_item_count = 7}}, preselect = cmp.PreselectMode.None, formatting = {format = _2_}, completion = {keyword_length = 2}})
