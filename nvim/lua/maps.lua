...
vim.keymap.set({"n", "v"}, "<leader>j", _11_, {silent = true})
local function _12_()
  local hop = require("hop")
  local hint = require("hop.hint")
  return hop.hint_lines({direction = hint.HintDirection.BEFORE_CURSOR})
end
vim.keymap.set({"n", "v"}, "<leader>k", _12_, {silent = true})
local function _13_()
  return (require("hop")).hint_char2()
end
vim.keymap.set({"n", "v"}, "s", _13_, {silent = true})
local function _14_()
  return (require("hop")).hint_char2()
end
vim.keymap.set({"o"}, "z", _14_, {silent = true})
